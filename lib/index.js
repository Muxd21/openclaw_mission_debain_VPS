...
Commands:
  ...
  update      Update OpenClaw and Mission Control
...
Examples:
  ...
  openclawx update            # Update everything
...
function showStatus() {
  ...
  if (status.proot && status.ubuntu && status.openClawInProot) {
    ...
    console.log('  openclawx update      # Update everything');
...
function startGateway() {
  ...
  const gateway = runInProot('(cd /opt/mission-control && PORT=3000 pnpm start) & openclaw gateway --verbose');
...
function runUpdateCmd() {
  ...
  console.log('Pulling updates for OpenClaw and Mission Control...');

  // Update npm package openclaw globally
  // Update mission-control from git
  const updateScript = `
    npm install -g openclaw@latest
    if [ -d "/opt/mission-control" ]; then
      cd /opt/mission-control
      git pull
      pnpm install
      pnpm run build
    else
      git clone https://github.com/builderz-labs/mission-control /opt/mission-control
      cd /opt/mission-control && pnpm install && pnpm run build
    fi
  `;

  const proc = runInProot(updateScript);
  proc.on('close', (code) => {
    console.log(`Update completed with code ${code}`);
    console.log('Now updating the installer itself...');
    import('child_process').then(({ spawn }) => {
      const updateCmd = 'cd ~/.openclaw-installer && git pull origin main && npm install -g .';
      const installerUpdate = spawn('bash', ['-c', updateCmd], { stdio: 'inherit' });
      installerUpdate.on('close', (c) => {
        console.log(`Installer update completed with code ${c}`);
      });
    });
  });
}
